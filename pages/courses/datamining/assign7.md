...
.. slug: dm_assign7
.. date: 2024-11-20 12:23:01 UTC-04:00
.. tags:
.. category:
.. link:
.. description:
...
testing data, so that the remaining data is training. 
...
Implement the linear regression algorithm via QR factorization, namely Algorithm 23.1 on
page 602 in Chapter 23. Make sure you augment $\mathbf{X}$ by adding a columns of ones as
the first dimension.

You must implement QR factorization on your own, as described in Section 23.3.1. You
...
correct is to ensure that $\mathbf{Q}\mathbf{R} = \mathbf{D}$.

Next, using the $\mathbf{Q}$ and the $\mathbf{R}$ matrices, you must solve for the
...
using those). See Example 23.4 on how backsolve works.
...
**CSCI6390**: You will implement the QR factorization as above, including backsolve.
...
$n \times (d+1)$, then append to it a diagonal matrix $\alpha \times \mathbf{I}$ of size
$(d+1)\times (d+1)$. This way the new training data will be of size $n+(d+1) \times (d+1)$. 
...
Note that even though this dataset is binary classification task, please treat it as a
K-way classification, where K=2. 
...
F1-score (see Eq 22.7 in chapter 22; you can use scikit.learn in-builts for this).
